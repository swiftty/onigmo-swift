...
@preconcurrency @testable import COnigmo
...
struct OnigmoError: Error {
    ...
    init(status: Int32) {
        ...
        let format = onig_error_code_to_format(self.status)
...
class COnigmoTests {
    ...
    func `check simple regex`() throws {
        ...
        var utf8Encoding = OnigEncodingUTF_8
        ...
        func search(_ string: String) throws -> [Range<String.Index>] {
            ...
            return string.withOnigUCharString { pointer, length in
                    ...
                    result >= 0
                {
                    ...
                    ranges.append(range)
                    start = start.advanced(by: region.pointee.end.pointee)
                }
